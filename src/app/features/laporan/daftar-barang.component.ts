...
export interface DaftarBarangItem {
...
export interface DaftarBarangRekapitulasi {
  baik: number;
  rusakRingan: number;
  rusakBerat: number;
  total: number;
...
export interface DaftarBarangData {
  tahun: number;
  items: DaftarBarangItem[];
...
export class DaftarBarangComponent implements OnInit {
  ...
  protected readonly data = signal<DaftarBarangData | null>(null);
  ...
  private loadData(tahun: number) {
    ...
    this.data.set(null);
    this.http
      .get<any>(`${this.env.apiBaseUrl}/vehicles/daftar-barang`, {
      ...
      .subscribe({
        ...
        error: (err) => {
          this.error.set(err?.error?.message ?? 'Gagal memuat data daftar barang.');
          this.loading.set(false);
        },
      });
  }
  ...
  protected async exportPdf() {
    ...
    try {
      ...
      pdf.save(`Daftar-Barang-Kendaraan-${tahun}.pdf`);
