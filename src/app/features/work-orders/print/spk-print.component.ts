...
import { WorkOrder, Image } from '@shared/models';
...
export class SpkPrintComponent implements OnInit, OnDestroy {
   ...
   * The src attribute is expected to be a signed R2 URL; we extract
   * the imageId from the data-image-id attribute set on each img tag.
   */
  private async cloneWithInlineImages(el: HTMLElement): Promise<HTMLElement> {
    const clone = el.cloneNode(true) as HTMLElement;
    // Get the original images from the live DOM to read data-image-id
    ...
    await Promise.all(
      liveImgs.map(async (liveImg, i) => {
        ...
        if (imageId) {
          ...
          // No imageId — try direct fetch (may fail due to CORS)
          try {
            const blob = await firstValueFrom(
  ...
  protected async exportPdf() {
    ...
    try {
      ...
      for (let i = 0; i < printPages.length; i++) {
        ...
        // Replace external images with inline data URLs in a hidden clone
        ...
        // CRITICAL: Inject override BEFORE appending to DOM
        // This neutralizes oklch() before html2canvas parses styles
      ...
      if (this.pdfAttachmentDefs.length === 0) {
        pdf.save(`SPK-${this.wo()?.nomor ?? 'dokumen'}.pdf`);
        this.isExporting.set(false);
        return;
      }

      // Fetch all PDF attachments via backend proxy endpoint (no CORS)
      const spkPdfBytes = pdf.output('arraybuffer');
      ...
      forkJoin(fetchAttachments)
        .pipe(
          switchMap(async (attachmentBuffers) => {
            ...
            const spkDoc = await PDFDocument.load(spkPdfBytes);
            const spkPages = await mergedDoc.copyPages(spkDoc, spkDoc.getPageIndices());
            spkPages.forEach((p) => mergedDoc.addPage(p));
        ...
        .subscribe({
          next: (mergedBytes) => {
            ...
            a.href = url;
            a.download = `SPK-${this.wo()?.nomor ?? 'dokumen'}.pdf`;
            a.click();
  ...
  protected totalShs(acc: number, item: { hargaVendor: number | string }): number {
